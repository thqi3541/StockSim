...
public class MongoDBDocumentParser {
     ...
     * <p>
     * The method uses reflection to map the fields in the {@link Document} to the
     * fields of the specified class. It supports nested objects by recursively parsing
     * fields that are represented as {@link Document} in MongoDB. The constructor
     * matching the fields of the class is used to create the instance.
     * </p>
     *
     ...
     * @throws DocumentParsingException If the parsing process encounters errors,
     *                                  such as missing fields or invalid field types.
     */
    public static <T> T fromDocument(Class<T> cls, Document doc) throws DocumentParsingException {
        try {
            ...
            e.printStackTrace();
            throw new DocumentParsingException("Error parsing document to " + cls.getName(), e);
        }
    }
    ...
    private static boolean doesConstructorMatch(Parameter[] parameters, Document doc) {
        ...
        return paramNames.containsAll(docFieldNames) && docFieldNames.containsAll(paramNames);
    }
    ...
    private static Object parseParameter(Parameter param, Document doc) {
        ...
        try {
            ...
            if (value instanceof Document) {
                return fromDocument(param.getType(), (Document) value);
            }
                ...
                if (param.getParameterizedType() instanceof ParameterizedType parameterizedType) {
                    ...
                    if (genericType.getPackageName().startsWith("entity")) {
                        return StreamSupport.stream(iterable.spliterator(), false)
                                .map(item -> {
                                    try {
                                        return item instanceof Document ? fromDocument(genericType, (Document) item) : item;
     ...
     * <p>
     * The method uses reflection to iterate through all fields of the provided object.
     * Fields that are nested objects are recursively converted into nested {@link Document}.
     * Primitive fields and supported types (e.g., {@link String}, {@link Integer}, {@link Date})
     * are added to the {@link Document} directly.
     * </p>
     *
    ...
    public static <T> Document toDocument(T object) throws DocumentParsingException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new DocumentParsingException("Failed to parse object into document ", e);
        }
        return doc;
    }
    ...
    private static boolean isDocument(Object value) {
        ...
        return objectPackage != null && objectPackage.getName().startsWith("entity");
    }
