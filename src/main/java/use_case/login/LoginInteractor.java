...
public class LoginInteractor implements LoginInputBoundary {
    ...
    public LoginInteractor(LoginDataAccessInterface dataAccess,
        ...
        ServiceManager.Instance()
                      .registerService(LoginInputBoundary.class, this);
    }

    @Override
    public void execute(LoginInputData data) {
        try {
            User currentUser = dataAccess.getUserWithPassword(data.username(),
                                                              data.password());
            String credential =
                    SessionManager.Instance().createSession(data.username());
            ...
            outputPresenter.prepareSuccessView(
                    new LoginOutputData(currentUser));
