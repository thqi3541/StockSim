...
class ViewHistoryInteractorTest {
    ...
    void successTest() throws ValidationException {
        ...
        ViewHistoryInteractor interactor =
                new ViewHistoryInteractor(dataAccess, outputPresenter);
        interactor.execute(inputData);

        // check if message is prepared
        verify(outputPresenter)
                .prepareSuccessView(new ViewHistoryOutputData(
                        mockUser.getTransactionHistory()));

        // check if user portfolio is empty
        TransactionHistory mockUserTransactionHistory =
                mockUser.getTransactionHistory();
        List<Transaction> userTransactions = new ArrayList<>();
        assertEquals(userTransactions,
        ...
        Transaction mockBuyTransaction =
                new Transaction(buyTimestamp, ticker, 10, 100.0, "buy");
        ...
        Transaction mockSellTransaction =
                new Transaction(sellTimestamp, ticker, 5, 100.0, "sell");
