...
- Implemented periodic update of stock information from data access interface in
  `StockMarket` that self-adjusts based on API rate limit
...
- Implemented `StockDataAccess` to retrieve current market prices for set
  tickers
...
- Added `SessionManager` to validate client requests by credential and manage
  active sessions
